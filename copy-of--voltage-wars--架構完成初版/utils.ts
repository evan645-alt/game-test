...
export const playGameSound = (type: 'click' | 'draw' | 'place' | 'wire' | 'attack' | 'victory' | 'alert') => {
    try {
        if (!audioCtx) {
        ...
        switch (type) {
            ...
            case 'victory': // Major arpeggio
                const playNote = (freq: number, startTime: number) => {
                    ...
                    g.gain.linearRampToValueAtTime(0, startTime + 0.3);
                    o.start(startTime);
...
export const getPhaseInstruction = (phase: GamePhase, activeTeamName?: string): TrilingualContent => {
    ...
    switch (phase) {
        ...
        case GamePhase.COIN_TOSS:
            return {
                zh: "決定本回合的攻擊順序！",
                en: "Determining Turn Order for this Round!",
                ja: "このラウンドの攻撃順を決定！"
            };
        ...
        case GamePhase.B_ACTION_1:
            return {
                zh: `【階段一：強制攻擊】${name}，必須對對手使用此卡。`,
                en: `[Stage 1: Mandatory Attack] ${name}, you MUST use this card on the opponent.`,
                ja: `【ステージ1：強制攻撃】${name}、このカードを対戦相手に使用しなければなりません。`
            };
        case GamePhase.A_ACTION_1:
            return {
                zh: `【階段一：強制攻擊】${name}，你現在必須攻擊對手。`,
                en: `[Stage 1: Mandatory Attack] ${name}, you must now ATTACK the opponent.`,
                ja: `【ステージ1：強制攻撃】${name}、対戦相手を攻撃しなければなりません。`
            };
        case GamePhase.B_ACTION_2:
            return {
                zh: `【階段二：自我強化】${name}，必須對自己使用此卡。`,
                en: `[Stage 2: Mandatory Buff] ${name}, you MUST use this card on YOURSELF.`,
                ja: `【ステージ2：自己強化】${name}、このカードを自分自身に使用しなければなりません。`
            };
        ...
        case GamePhase.B_ACTION_3:
            return {
                zh: `【階段三：自由選擇】${name}，攻擊對手、強化自己或跳過。`,
                en: `[Stage 3: Flexible] ${name}, Target Opponent, Self, or Skip.`,
                ja: `【ステージ3：自由選択】${name}、対戦相手を攻撃、自分を強化、またはスキップできます。`
            };
        case GamePhase.A_ACTION_3:
            return {
                zh: `【階段三：自由選擇】${name}，這是最後的機會。攻擊、強化或跳過。`,
                en: `[Stage 3: Flexible] ${name}, final chance. Attack, Buff, or Skip.`,
                ja: `【ステージ3：自由選択】${name}、最後のチャンスです。攻撃、強化、またはスキップしてください。`
            };
